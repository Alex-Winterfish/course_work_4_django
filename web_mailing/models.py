...
class MailingModel(models.Model):
    ...
    status = models.CharField(
        choices=STATUS_IN_CHOICES, default=CREATED, verbose_name="статус рассылки"
    )
    message = models.ForeignKey(
        MessageModel, on_delete=models.CASCADE, verbose_name="сообщения для отправки"
    )
...
class MailingAttemptModel(models.Model):
    ...
    server_feedback = models.CharField(
        max_length=500, verbose_name="ответ почтового сервера"
    )
    mailing = models.ForeignKey(
        MailingModel, on_delete=models.CASCADE, verbose_name="рассылка"
    )
    owner = models.ForeignKey(
        CustomUser,
        null=True,
